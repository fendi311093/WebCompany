    ...
    <div class="relative z-10">
        <div class="w-full max-w-5xl px-4 xl:px-0 py-10 lg:pt-16 mx-auto">
            <div class="inline-flex items-center">
                <!-- Logo -->
                @if (isset($profilCompany) && $profilCompany->logo)
                    <img src="{{ asset('storage/' . $profilCompany->logo) }}"
                        alt="{{ $profilCompany->name ?? 'Logo Perusahaan' }}"
                        class="h-12 w-auto max-w-[160px] object-contain" />
                @else
                    <span class="text-neutral-400 italic">No Logo</span>
                @endif
                <div class="border-s border-neutral-700 ps-5 ms-5">
                    <p class="text-sm font-semibold text-neutral-400">
                        © {{ date('Y') }} {{ $profilCompany->name_company ?? 'Delta Kontainer Depot' }}
                    </p>
                </div>
                <!-- End Logo & Nama -->
            </div>
